...
func (pl *GPUFit) Filter(ctx context.Context, state fwk.CycleState, pod *v1.Pod, nodeInfo fwk.NodeInfo) *fwk.Status {
...
func (pl *GPUFit) Score(ctx context.Context, state fwk.CycleState, pod *v1.Pod, nodeInfo fwk.NodeInfo) (int64, *fwk.Status) {
	nodeInfo, err := pl.frameworkHandler.SnapshotSharedLister().NodeInfos().Get(nodeInfo.Node().Name)
	if err != nil {
